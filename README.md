...
[![ShellCheck](https://github.com/YOUR-USERNAME/packettracer-arch-installer/actions/workflows/shellcheck.yml/badge.svg)](https://github.com/YOUR-USERNAME/packettracer-arch-installer/actions/workflows/shellcheck.yml)
...
```bash
git clone https://github.com/YOUR-USERNAME/packettracer-arch-installer.git
cd packettracer-arch-installer
...
<img width="281" height="150" alt="how-it-works" src="https://github.com/user-attachments/assets/0fdf63dd-6652-4b48-8180-01eb504be63e" />
![Uploading how-it-works.svg…]()


1. **Extract:** unpacks the `.deb` into a temporary directory (removed when the script exits).
2. **Locate:** finds the AppImage inside the package. If there is none, the script prints the package contents and stops before changing anything on your system.
3. **Dependency:** installs `fuse2`, which AppImages need to run.
4. **Copy:** copies the package's `opt` files into `/opt` and marks the AppImage executable.
5. **Launcher:** creates `/usr/local/bin/packettracer`, a small wrapper that starts the AppImage.
...
Issues and pull requests are welcome. If something fails, please include your Arch version, the exact command, the full terminal output, and the output of `--inspect`. The script is checked with [ShellCheck](https://www.shellcheck.net/); run `shellcheck packettracer_installer.sh` before opening a pull request.
